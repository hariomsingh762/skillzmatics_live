                    ...
                    <div class="container-fluid">
                        ...
                        <div class="page-title-box">
                            <div class="row align-items-center">
                                <div class="col-md-8">
                                    <h6 class="page-title">All Unit</h6>
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap datatable-skills" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                            ...
                                            <tbody>
                                                @foreach($skillUnitList as $data)
                                              
                                                <tr id="row-{{ $data->id }}">
                                                    <td>{{$data->unit_name}}</td>
                                                    <td>{{$data->unit_slug}}</td>
                                                    <td>
                                                        <div class="col-lg-4">
                                                        <div>
                                                    ...
                                                    <td>
                                                        <a href="{{url('/edit-skills-unit')}}/{{$data->iid}}" class="btn btn-primary btn-sm">Edit</a> 
                                                        <button class="btn btn-danger btn-sm delete-skill-button" data-id="{{ $data->iid }}">Delete</button>
...
$(document).ready(function() {
    $('.status-switch').on('change', function() {
        ...
        $.ajax({
            url: '/update-skill-unit-status/' + skillId,
            type: 'POST',
            data: {
                "_token": "{{ csrf_token() }}",
                "status": newStatus
